...
## Run conditioning (edit these five lines each run)

```
BATCH SIZE:      20              # ignored for 6-ultra — that band emits ~6 by design
MOOD FAMILY:     special / use focal color
FOCAL COLOR:
dark_muted_cyan	#16505a
dark_vivid_teal	#005449
dark_muted_teal	#1a5249
dark_vivid_cyan	#00515c
COMPLEXITY BAND: 3-4 / 5             # one of: 1-2 / 3-4 / 5 / 6-ultra
VALUE KEY:       span            # one of: low / mid / high / span
```

the 'special' mood family means the following: the purpose of this palette is to generate an image where a specific focus color is prominent. Color matches are not exact but cover a range. I have provided this range in several color spaces, you do not need to "count" what percentage of the generated palettes covers this range, but know that it should be prominent in each palette and generated images should have at least ~10-20% of the image in ths focal color so it needs to cover a range. But the palette should still otherwise still be dramatic and interesting and can include other colors that complement the focus color well. when multiple focal colors (N) are given, each focal color should appear in at least 1/N of the palettes -- there's no bias to make or not make palettes that combine the two.
