# Dramatic Fractal Palette Generator — v3.2

```
CLASSIC_PALETTE RANGE:      specified in promt, STOP if not specified              # the index of palettes under consideration in classic_palettes.json
PALETTES PER ENTRY: 2
PALETTE COUNT: 
MOOD FAMILY:     special / use classic_palettes.json
COMPLEXITY BAND: c3-4
VALUE KEY:       special
```

## Goal — classic-entry mode

Take the entries in classic_palettes.json at the indices in CLASSIC_PALETTE RANGE and
create PALETTES PER ENTRY palettes for each one. Every palette is a *rendering of that
entry*: when the fractal is drawn, it must feel unmistakably like those 2–3 colours and
nothing else. The general rules below (drama, skeletons, routing, validation) still
apply, but they are subordinate to this one requirement.

**The hue inventory is closed.** The only chromatic hues allowed are the entry's own
hues (±15° in OKLCH). Everything else is achromatic — black, white, grey, ivory, bone —
at C ≤ 0.04. Do not introduce a third hue as a bridge, a shadow, a "ground", or a
glow tint: a blue/orange palette gets no violet dark and no peach mid; if a transition
needs a bridge, bridge through an achromatic or through a lighter/darker version of
one of the source hues. (The classic failure: two source hues plus a violet ground plus
a peach mid plus large black and white fields — a five-colour palette that reads as
"muddy sunset", not "blue/orange".)

**Most of the ramp is source colour.** At least ~70% of the palette (by position)
should sit ON one of the entry's hues at recognizable chroma (≥ 0.5× the source's
chroma). Black and white are waypoints, not fields: a single stop each, with the
neighbouring stops close (≤ 0.10 pos away) and already carrying a source hue. Never
more than ~15% of the ramp near-achromatic in total.

**Spend the stop budget on intensity, not on new hues.** c3–4 gives 6–9 stops; use
them to explore the *same* hue at different lightness and chroma — for blue:
ink-navy → cobalt → sky → ice-white; for orange: burnt umber → pure orange →
peach-cream. A stop that is the same hue at a different L/C is a real event. This is
where the drama comes from in this mode: wide value range within each hue family,
and the tension between the two families.

**Which colour dominates, and where black/white sit, is what varies between the
PALETTES PER ENTRY palettes** — not the hue set. The two palettes for one entry should
differ in skeleton, value key, dominant hue, and glow temperature (warm-tinted vs
cool-tinted white, still C ≤ 0.04), and should share every chromatic hue.

**Three-colour entries** must show all three; typically the third is the dark
(Magenta Cyan's violet-black) or the pale (Green Gold Ivory's ivory) and takes the
ground/glow role instead of an achromatic.

**Architectures**: pick from those the entry can honestly support — duotone-plus,
complementary-tension and near-monochrome-spark (spark = the other source hue) work
for any two-colour entry; triad for three-hue entries; mono-temperature-ramp only
when the entry is one temperature (Coffee, Autumn Spice); analogous-drift only when
the entry's hues are neighbours (Sea Ladder, Sunset Bliss). Do not force the six-way
spread; do spread skeletons and value keys.

**Routing** still holds: a jump between two source hues > 55° apart goes through a
black or white waypoint, or through the near-white / near-black end of one of the
source hues (orange → pale peach-white → ice-blue → cobalt stays in family). Cliffs
are a small step within one hue.

**Hexes**: you need not hit the exact hex codes, but the entry's colours should each
appear recognizably — validator: `--classic classic_palettes.json --range A-B`.

**Names**: unique; may derive from the entry name.

**Workflow**: convert the entry hexes to OKLCH first (the validator has hex_to_oklch);
author all palettes in one script; run `python3 validate_palettes.py <batch>
--classic classic_palettes.json --range A-B --sheet <batch>.png`; fix every ERROR;
then look at the sheet — each row should look like the swatches beside it. If a row
looks like a different palette from its swatches, the hue inventory leaked; fix that
before anything else.

**Output**: the JSON file named `classic_A-B_c3-4_v3_1.json`, plus one or two sentences
on how it went. No other commentary.

**Validator backstop to add (classic mode)**: ERROR any stop with C > 0.05 whose hue
is > 25° from every source hue of its entry; warn when < 70% of sampled positions
sit on a source hue at ≥ 0.5× source chroma; warn when any run of near-achromatic
positions (C < 0.04) exceeds 0.15 of the ramp.


no meaningful commentary in the output needed, just the output json and a sentence or two summarizing how things went.
