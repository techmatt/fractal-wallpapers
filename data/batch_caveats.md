...
## NOT-A-TIER — `repeat_ab_ckpt121` is a COMPARATIVE scale in its own store

**A `3` here means *the repeat is the better picture*, not tier 3.** The
`repeat_ab` store's ordinals are `1` the repeat is worse, `2` neutral, `3` the
repeat is better, cast on a composite tile carrying two renders of one place. They
are not the 1–4 quality scale, they are not a rescaling of it, and there is no
offset or transformation that makes the two commensurable — they answer different
questions. **These verdicts may never enter `smooth_render`, `strange_render` or
`gallery_grade`, as themselves or transformed**, and nothing trains on them
without Matt saying so. The structural guard is that an attribute row carries
`class` and `labeling.attributes.check` refuses one carrying a `score`, so the
confusion cannot be created by pooling the stores on field name.

**The side order is FIXED and is the other thing a reader can get backwards.** The
1× render is always on the **left** and the repeat always on the **right**, on
every tile of the sitting, at Matt's instruction. The sheet manifest records it in
`render.sides`; a reading that assumed the halves alternated would report the
opposite result with no sign that anything was wrong.

**The population is CONDITIONAL and that is the batch rather than a defect.** Every
unit is a smooth-routed candidate-ledger row reading `p_fine(>=4) >= 0.030242`
(`solve.DEFAULT_FINE_BAR`) under `twelve_sheets_drop_high_asymmetric_auc_ge4_more_k3`
at candidate geometry — 2,549 such rows at 2,082 places, 250 drawn at seed
20260911, one per location. So **a rate measured here is a rate about pictures that
are already good at 1×, and is a ceiling on any rate about the pool**: the sitting
asks whether repetition improves a picture that already works, which is the only
repetition Matt will consider after `repeat_axis_*_20260911` closed it as a general
draw. Nothing here says what repetition does below the bar; `repeat_axis_*` is the
batch that asked that, and its answer was −0.273 tiers.

**One rung, and it is two different doses.** `cycles = 2` on every tile, `phase`
held at 0 on both halves. On a cyclic map that is a **2×** traversal and on a
folded (sequential) one it is **4×**, because a folded map is baked as an
out-and-back — 189 tiles are cyclic and 61 are folded, so the two arms are 189 and
61 and not a balanced design. The split is whatever the population above the bar
holds (folded is 20.3% of it) rather than the deliberate over-share
`repeat_axis_*` used, and it is therefore itself a reading.

**No row on this page carries a human verdict in either finished store.** 273
candidates above the bar were dropped for that, which is the `aug_sweep_A` failure
being avoided: a page mixing already-judged rows against fresh ones cannot be
repaired afterwards, because a difference between the two halves could be the axis
or the history. The exclusion is per **render key** and not per place, so 421 of
the 250 drawn places do carry a verdict under some other colouring — a per-location
read across this sitting and the finished stores is a read over two colourings of
one place.

**Nothing on the page was prefilled by a head and nothing was ordered by one.**
Every unit starts at `2`, stated by the plan, and the page is a seeded shuffle.
Both are deliberate deviations from this rig's correction-sheet default and both
are recorded on the registration: the premise of the sitting is that the heads
cannot read this axis, so a score order or a head prefill would have imported that
error into page position and into the anchor. `anchored` is `false` for exactly
that reason and it is true rather than lenient. The render judge did read both
halves at label geometry and its readings travel on the row under `reading`, where
the page never sees them.
