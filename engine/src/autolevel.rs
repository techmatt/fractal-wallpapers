//! `band_autolevel/v1`, as the stops surgery it is.
//!
//! **Two copies of this file exist, on purpose and for now.** This one is the
//! engine's; the other is `explorer/engine-wasm/src/level.rs` in the
//! `fractal-website` checkout, which this was copied from. That repository is the
//! busy one, so it keeps its copy until a later website prompt switches engine-wasm
//! to import this module and deletes its own. Until then an edit here is an edit
//! there, and `the_port_is_the_operator` and `the_derivation_is_the_operator` at the
//! foot of this file read that crate's fixtures, read-only, to hold the two to the
//! same answers. What differs between them is below the band constants: this copy
//! reads the band out of the tracked `data/coloring/levels_band.json` at compile
//! time, where that one transcribed it.
//!
//! **The operator is not a second operator.** In this repository it is Python —
//! `coloring/autolevel.py` — and it works in two halves that are worth keeping
//! apart, because only one of them is here:
//!
...
//!
//! **It acts on the colormap because the operator does.** A map is baked from
//! control points by [`crate::colormap::Colormap::from_stops_baked`]; curving the
//! stops before that call is the same edit the operator makes when it writes a
//! levelled colormap file and points `colormap_dir` at it. Nothing downstream can
//! tell the difference, which is the point — and it is why `render-link` replays a
//! link's `level=` here rather than writing a file.
//!
//! **Ottosson's matrices are the engine's, not a copy.** Every conversion here
//! goes through [`crate::colormap`]'s own public `srgb_to_linear`,
...
impl Curve {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        // Both are finite by now, so `<=` is the whole of "not above".
...
/// which is a JSON-legibility step taken on the way to disk; there is no disk here.
/// **The two bakes are therefore not bit-identical by construction**: a knot moved by
/// up to 5e-10 moves the interpolation fraction of the table samples beside it, by
/// about 1e-9 in linear light, and an output byte changes only where that lands on a
/// rounding boundary. A replica bake over every map found 0 of 4,829,184 entries move
/// their sRGB8 byte (`cpu_default_and_rust_audit_ckpt148`).
...
/// The tracked band record, compiled in: `data/coloring/levels_band.json`, whose
/// sha256 every recorded stamp names. A different band is a different operator's
/// answer, and a gallery seat levelled under one would stop agreeing with a view
/// levelled under another — so the bytes are the file's and are never retyped.
const BAND_RECORD: &str = include_str!("../../data/coloring/levels_band.json");
...
/// [`BAND_RECORD`]'s three `bands.<statistic>.band` pairs.
///
/// Read out of the text and parsed by `str::parse`, which is correctly rounded, rather
/// than through `serde_json`'s float reader, which without `float_roundtrip` may land a
/// seventeen-digit double one unit of last place away — and a band one ulp off is a
/// curve that is not the operator's. `the_band_is_the_tracked_file` pins the three
/// pairs to the doubles the website's copy transcribed.
fn band() -> &'static Band {
    static BAND: OnceLock<Band> = OnceLock::new();
    BAND.get_or_init(|| {
        let pair = |statistic: &str| {
            band_pair(BAND_RECORD, statistic).unwrap_or_else(|| {
                panic!("levels_band.json has no readable bands.{statistic}.band pair")
            })
        };
        Band {
            black: pair("black_pt"),
...
/// The `[low, high]` after `"bands"`, then `"<statistic>"`, then `"band"`, as text.
fn band_pair(text: &str, statistic: &str) -> Option<[f64; 2]> {
    let bands = &text[text.find("\"bands\"")?..];
    let entry = &bands[bands.find(&format!("\"{statistic}\""))?..];
    let band = &entry[entry.find("\"band\"")?..];
    let open = band.find('[')?;
    let close = band.find(']')?;
    let mut numbers = band[open + 1..close]
        .split(',')
        .map(|number| number.trim().parse::<f64>().ok());
    let pair = [numbers.next()??, numbers.next()??];
    numbers.next().is_none().then_some(pair)
}
...
fn percentile(values: &mut [f64], q: f64) -> f64 {
    // numpy's plotting-position constants for the `linear` method, named so the
    // expression below is numpy's own rather than a folded `-1`.
    const ALPHA: f64 = 1.0;
    const BETA: f64 = 1.0;
    let n = values.len();
    let quantile = q / 100.0;
    let virtual_index = n as f64 * quantile + (1.0 + quantile * (1.0 - ALPHA - BETA)) - 1.0;
...
pub fn derive_curve(statistics: &ToneStats) -> Decision {
    ...
    let low = match measured_black {
        None => black,
        Some(value) => project(value, band.black),
    };
    let high = project(white, band.white);
    let target = project(middle, band.mid);
    ...
    // `max` then `min` rather than `clamp`, which would pass a NaN through where this
    // lands on the lower bound — and which is `numpy.clip`'s order of the two.
...
#[cfg(test)]
mod tests {
    use super::{Curve, Decision, ToneStats, band, curved_stops, derive_curve, tone_stats};
    use std::path::PathBuf;

    /// The website's engine-wasm crate, whose fixtures pin its copy of this file. Read
    /// and never written; every test that needs it skips by name where it is absent.
    fn website_crate() -> PathBuf {
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../fractal-website/explorer/engine-wasm")
    }

    fn derive_cases() -> Option<serde_json::Value> {
        let fixture = website_crate().join("level-derive-cases.json");
        let Ok(text) = std::fs::read_to_string(&fixture) else {
            eprintln!("no {}; skipped", fixture.display());
            return None;
        };
        Some(crate::exact_json::exactly(&text))
    }

    /// **The band is the tracked file, and it is the band the website's copy transcribed.**
    /// Those doubles are the ones `level.rs` carries as constants, cited there against the
    /// file's sha256; a band that read differently here would be a second operator.
    #[test]
    fn the_band_is_the_tracked_file() {
        let band = band();
        assert_eq!(band.black, [0.0, 0.3008176686683185]);
        assert_eq!(band.white, [0.8629886965307019, 0.9960350764349456]);
        assert_eq!(band.mid, [0.2663290436868155, 0.7380826210003913]);
    }
    ...
    fn the_derivation_is_the_operator() {
        let Some(cases) = derive_cases() else { return };
    ...
    fn the_derivation_is_the_operator_on_every_backfill_row() {
        let sidecar = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("../artifacts/curation/autolevel_backfill.jsonl");
        ...
        for line in text.lines().filter(|line| !line.trim().is_empty()) {
            let row = crate::exact_json::exactly(line);
    ...
    fn the_measurement_is_the_operator() {
        let Some(cases) = derive_cases() else { return };
        let pixels = website_crate().join("../../artifacts/level-derive");
    ...
    ///
    /// Regenerated, when a case has to move, by the operator in this checkout:
    ///
    /// ```text
    ...
    /// ```
    ///
    /// The gradients are this repository's `data/palettes`; the fixture is the website
    /// crate's, read in place, so this test is skipped where that checkout is not beside
    /// this one rather than failing — which is every CI runner.
    ///
    ...
    fn the_port_is_the_operator() {
        let library = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../data/palettes");
        let fixture = website_crate().join("level-cases.json");
        let Ok(text) = std::fs::read_to_string(&fixture) else {
            eprintln!("no {}; skipped", fixture.display());
            return;
        };
        let cases: serde_json::Value =
            serde_json::from_str(&text).expect("level-cases.json parses");
