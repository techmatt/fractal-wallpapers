...
//!
//! **Neither is a second renderer.** The weight reads the lanes a shade already holds,
//! through the engine's own [`Stretch`]. The probe is the one place this module walks an
//! orbit itself — `Painter::trace` returns a colour, and the question here is how many
//! samples went into it — so its hit test is written to be `trace`'s line for line, and
//! the tests at the foot of this file hold the two to each other.
//!
...
//!
//! **Two copies of this file exist, on purpose and for now.** This is the engine's; the
//! other is `explorer/engine-wasm/src/derive.rs` in the `fractal-website` checkout, which
//! it was copied from, and which that crate keeps until a later website prompt imports
//! this one instead. The only difference is the inflection pre-map, a feature of that
//! crate that is off in its shipped build: [`probe_row`] here starts every orbit at the
//! viewport's own point, which is what that build's identity pre-map does. `render-link`
//! does not call this module — a link carries the value it was drawn at, and a v3+ link
//! that leaves a derived parameter out is refused rather than derived off a canvas the
//! command does not have.

use crate::coloring::{Blend, Coloring, Stretch, Transform};
use crate::colormap::Colormap;
use crate::direct_trap::{Painter, Shape};
...
pub fn weight(roughness: f64, ceiling: f64) -> f64 {
    if roughness.is_nan() || roughness <= 0.0 {
        return ceiling;
    }
...
pub fn opacity_cap(shape: Shape, merge: Blend) -> f64 {
    if merge == Blend::Screen && shape == Shape::Cross {
        crate::direct_trap::SCREEN_CROSS_OPACITY_CAP
    } else {
        1.0
    }
}
...
mod tests {
    use super::*;
    use crate::coloring::MergeOrder;
    use crate::colormap::{Bake, Kind};
