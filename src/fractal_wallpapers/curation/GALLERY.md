...
**This file states what is true now; the arguments that reached it live in
[`GALLERY_decisions.md`](GALLERY_decisions.md).** Every retirement, audit, flip and
before/after moved there on 2026-09-12, so a rule here that was argued into
place cites the heading that argued it rather than carrying the argument — and a
heading in that log is never renamed, because a citation names a heading and a file.

**A `centered` row is refined on its scale alone**, stage B off and three frames
instead of seven, and the refinement's live home is the walk's own close-of-run
leg (`supply/harvest.py`) rather than a seating — *The pre-solver gallery pass,
and what took its place* in [`GALLERY_decisions.md`](GALLERY_decisions.md).

## What may refuse a seat, before any leg tries to fill one

**The colour ceiling and its targets, and the binding reason an empty seat
records, are what the retired pre-solver pass established and what `curate solve`
still reads.** The four sections below are those rules; the pass itself, the
commands that went with it and what its four passes left behind are in that same
log.

⚠ **This heading is newer than the four under it**, added 2026-09-12 when the
pre-solver pass's own `##` went to the log and left them with no parent. It names
nothing the pass named, so no citation points at it and none of the four was
...
**The ratified bar keeps a quarter of the seatable pool and deletes its worst
fifth rather than raising its median** — *A `p_fine` bar on the view raises the
objective and deletes the worst fifth* in
...
**Which cells sit exactly at the colour floor is a property of the head and never
a standing list** — *The first seating on the adopted column, and what the
concentration cost* in [`GALLERY_decisions.md`](GALLERY_decisions.md).
...
**n=2000 is the size the collection is being built toward** (Matt), and a
shortfall is read against `curate headroom` and never on its own — *At the
planning size the binding constraint is distinctness, not places* in
[`GALLERY_decisions.md`](GALLERY_decisions.md).

**`solve.json`'s `diversity_refusals` is the twin rule's own refusal log**, so
auditing the threshold or pricing a wider radius needs no re-solve — *And the
threshold it binds at is too high* in
[`GALLERY_decisions.md`](GALLERY_decisions.md).

**The metric's own direction count is [`pixel_clouds.DIRECTIONS`] 256 and `TAU` is
not refitted for it** — *The twin metric's two costs, both now taken* in
[`GALLERY_decisions.md`](GALLERY_decisions.md).
...
**No shipped leg may reuse `Pairs.measure([at, *seated])`'s call shape**, and
`test_rules.py` guards it — *What was retired, and why* in
[`GALLERY_decisions.md`](GALLERY_decisions.md).
...
**The gate is on the manifest.** `config.theme_bar` carries the effective bar, the
...
`group_cap`, `P`) and the bar it ran under at `config.theme_bar`. The pool block
was called `theme` until 2026-09-12, which put a dict and a string under one name
in one record — and since `config` is the block a tracked manifest carries whole,
`config.get("theme")` looked exactly right, returned `"dark_vivid_green"`, and
every `.get()` on it raised. A record taken before that spells the pool block
`theme` and the bar `themed_bar`.
...
key still decides* in [`GALLERY_decisions.md`](GALLERY_decisions.md). Two of the
five have since moved, both on 2026-09-09
...
takes the seating key, and cuts clusters* in that same log.

**`gallery-grade score-pool` runs on coarse-clears only, so `p_fine` exists above
`solve.Q4_BAR` and nowhere else** and a site below that bar cannot switch keys —
*Where the coarse key still decides* in
[`GALLERY_decisions.md`](GALLERY_decisions.md).

**A pre-selection record carries `key`, `ordered_on` and
`places_on_the_fallback`**, because the two scales are stacked and unknown never
...
that by destroying every row at every absorbed place, and *The fold picks its
survivor on the seating key* in [`GALLERY_decisions.md`](GALLERY_decisions.md)
measured what that cost: 41.7% of folds destroyed a mode the survivor has no row
in and 95.1% a colour cell it cannot reach. Since 2026-09-09 it is a **relabel**.
...
**Pooling costs the diversity rule about 12% and `rules.Twins.record` carries
`seconds` so the next pool that grows can be read against it** — the two arms over
one pool are *Both folds over one pool, n=1000* in
[`GALLERY_decisions.md`](GALLERY_decisions.md), which is this section's own
before/after and sits there under *The fold picks its survivor on the seating
key*.

**`--locations N` keeps the N strongest CLUSTERS**, ranked under
[`distinct.offered_at`] after the fold, with `pool.truncation` on the record —
*The `--locations` cut takes the seating key, and cuts clusters* in
[`GALLERY_decisions.md`](GALLERY_decisions.md).
...
**A cap is asked of `ceiling` and never spelled a second time**, which is census
schema 4 — *The group cap was a second spelling, and the census was the one that
was wrong* in [`GALLERY_decisions.md`](GALLERY_decisions.md).
...
**`solve.ranking_for` is the one place a pass pays for its key**, off
`solve.DEFAULT_KEY` and `solve.DEFAULT_GROUP_CAP`, reading the flatness sidecar
and the location scores once per pool, and `seat(order=...)` overrides it — *Both
gallery decisions flipped on 2026-08-28, and the incumbent is still reachable* in
[`GALLERY_decisions.md`](GALLERY_decisions.md).
...
⚠ **`shortfalls.groups.counts_top20` is the busiest twenty and never all of them.**
The group count is `held`, in the same block. It is the **only** truncated `counts`
in the block: the sibling `cells.counts` and `families.counts` are complete, so `len`
agrees with `held` there and the habit picked up on those two is what made this one
wrong. The key was `counts` until 2026-09-12 and `len(counts)` read 20 on every pass
with more than twenty groups, which is every shipping pass; a record taken before
that spells it `counts` and is the same truncated twenty under the name that hid it.
...
picks its survivor on the seating key* in
[`GALLERY_decisions.md`](GALLERY_decisions.md). A place inside the radius of a place
