...
| `candidate_ledger.prune` → `retention.decide` | top-`RETAIN_PER_PAIR` per (location, mode) on the fitted `rank_key`, **plus `retention.FAMILY_ALLOWANCE`'s one further row for a colour family the kept five miss** (2026-09-15); a loser loses its row **and its JPEG** | **11.9%** of the rows it ranks. Only 7.1% of pairs hold two rows with a reading | **no**, and it is the only site here that destroys anything |
| `depth.best_field_by_location`, `mine.best_by_location` | which row is a place's *best*, which sets the band the near and deepen draws read | yes for the `[Q4_BAR, PRIMED_BAR)` band on `P(>=4)` — 0.50 to 0.90, entirely at or above `Q4_BAR` | yes — a draw spends renders and deletes nothing |
