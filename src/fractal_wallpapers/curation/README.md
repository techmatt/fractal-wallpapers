...
**This file states what is true now; the arguments that reach it live in
[`README_decisions.md`](README_decisions.md).** Nine sections that argued a change —
a renamed column, a repaired renderer, a rule replayed and refused, a pile of
pictures deleted — moved there verbatim on 2026-09-12, and a new one is appended
there rather than written back in here.
...
MEASUREMENTS.md  every standing quantity this stage relies on
```

**Each of those four has a decision log beside it**, split out on 2026-09-12: the
four above state what is true now, in the present tense, and the four below are the
append-only argument that reached it. A section moved **verbatim**, under its own
heading and in its original order, so a citation that named one still resolves —
which is why a heading in a log is never renamed once it is there. A new decision
is APPENDED to the log and the rule it settles is stated in the reference file.

```
README_decisions.md        a renamed column, a repaired renderer, a rule replayed
                           and refused, a pile of pictures deleted
GALLERY_decisions.md       the retirements, the audits, the flips, the before/afters
LEGS_decisions.md          what each leg's first sitting came to, in the order they ran
MEASUREMENTS_decisions.md  one run of one experiment a section: what it priced, what
                           it changed, and what a later reading bounded or reversed
```
...
**The prune ranks on [`rank_key`](rank_key.py) while a gallery seats on the cascade,
and below the bar those are one order** — `tests/test_seat_sheet.py` pins that
`_prune_ranks` names neither `solve.DEFAULT_KEY` nor the cascade, and
...
loop [`README_decisions.md`](README_decisions.md)'s *The 09-02 sweep and `curate
re-render` were a loop* records was found. The pool is smaller now and the sweep
no longer takes them, so neither half runs.
...
**Every renderer that draws a candidate passes the row's whole recipe, and one test
holds all eight to it** — `tests/test_renderer_agreement.py` renders one recipe
through each, compares the bytes, and sweeps the tree for `colorize.render` call
sites, so a renderer added later is either covered or declared exempt;
[`README_decisions.md`](README_decisions.md)'s *`mine.make` dropped `mode_params`, so
every varied candidate in the pool is the bare mode's picture* is the defect that
bought it.

**One builder turns a stored row into a release render** — `release.task_for`, whose
picture-deciding parameters have no defaults, and `release.Task(` is constructed
nowhere else in the tree; [`README_decisions.md`](README_decisions.md)'s *The fix was
one builder, not four more keywords* is why.

**`colorize.render` refuses a `fields=` directory beside a `curve` override alone**: a
palette override rides the field cache, and a map this repository has never rendered
before is served by that cache exactly as a shipped one is —
[`README_decisions.md`](README_decisions.md)'s *The `curve`/`palette` refusal was one
rule doing two jobs, and a NEW MAP is neither* carries the split. **Still unmeasured
there**: whether a levelled repaint composes with a non-identity `gamma` or
`transfer`.
...
**The subtraction on a card is `p_fine Δ` and never a margin**, the card names the leg
that placed the competitor beside it, and a record's rungs are read off its own
`config.fine_bar` and `config.fine_head` —
[`README_decisions.md`](README_decisions.md)'s *The subtraction on a card is not a
margin, and the card now says which leg placed the seat* carries the readings.
...
09-02 sweep and `curate re-render` became a loop — see
[`README_decisions.md`](README_decisions.md)'s *The 09-02 sweep and `curate
re-render` were a loop*. The record carries a `reference` block
...
**Whether the ten backfilled `runs` legs are worth keeping is still Matt's and still
open**, and what it is a question about is 11.75 GiB of superseded attempts of which
`pictures/` is 2.59 GiB: the `candidates/` cache under them went on 2026-09-07 as the
one free subtree, and `framings/` was kept unreferenced because its own
`framings.jsonl` names each trial by slug.
[`README_decisions.md`](README_decisions.md)'s *The 09-02 sweep and `curate re-render`
were a loop, and the union is what closed it* and *The 11.75 GiB is not the 14,995
pictures, and only one of its subtrees is free* carry the readings.
...
**A per-cell retention arm cannot prune this store** — a location carries median 1
mode against median 21 dominant cells, so two arms together reach almost every row it
holds, which is why the shipped rule is the mode arm alone;
[`README_decisions.md`](README_decisions.md)'s *A per-cell retention arm cannot prune
this store, and the shape says why* is the replay.
