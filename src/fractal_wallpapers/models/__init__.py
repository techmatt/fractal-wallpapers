...
The order is the order they run in, and each stage writes a record the next one
reads rather than being called from inside it — so a training run can be
re-scored, and a score can be re-judged, without any of it happening again.
"""
