...
**The newest reading is fast only**: `viewers_ckpt132` added 6 fast tests on 2026-09-19 and
read **4,828 fast in 153.96 s (2:34)**, 157 deselected, green, so the tree is *believed* at
4,985 collected; taken right after twenty-one record solves. Before it,
`mine_ckpt132_night_0730` added no test on
...
2026-09-18 and read **4,822 fast in 147.70 s (2:28)**, 157 deselected, green, so the tree is
*believed* at 4,979 collected; taken on a quiet box before its render leg. Before it,
`targets_raise_ckpt131` added no test on 2026-09-18 and
read **4,821 fast in 162.24 s (2:42)**, 157 deselected, green, count unmoved; taken right
after three collection records. Before it, `texture_draw_band_ckpt131` added 4 fast tests on
...
unmoved. Before it, `angle_modes_evening_ckpt130` added 19 fast tests on
2026-09-18 and read **4,816 fast in 155.31 s (2:35)**, 157 deselected, green, so the tree is
*believed* at 4,973 collected; it reads the +30 s below as box. Before it,
`add_three_mode_collections_ckpt130` added 3 fast
